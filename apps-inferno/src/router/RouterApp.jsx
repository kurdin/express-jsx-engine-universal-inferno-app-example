...
import { Switch, Route, Link, StaticRouter, withRouter } from 'inferno-router';
	...
	render({ location }) {
		return (
			<main>
				...
				{location.pathname !== '/' && (
					<a href="#" onClick={this.on.gotoHome}>
						Go Home (with onClick Example)
					</a>
				)}
			</main>
		);
...
class ShowTheLocation extends Component {
	render({ match, location, history }) {
		return <h3>Current Route: {location.pathname}</h3>;
	}
}
